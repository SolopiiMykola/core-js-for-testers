...
function asyncDoMath(a, b) {
  ...
  // there are no special key word for async function, read documentation of this function
  setTimeout(function() {
...
function printFileAsync(name) {
  const fs = require("fs");
  fs.readFile(`./6/data/${name}`, { encoding: "UTF8" }, function(err, content) { // the third parameter is callback
    console.log(name, "is ready:");
    console.log(content);
...
// There are a lot of librarys work with callbacks